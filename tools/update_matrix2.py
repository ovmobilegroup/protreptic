...
# Load name_to_modes from scenarios_zh.json
with open('scenarios_zh.json', 'r', encoding='utf-8') as f:
    zh = json.load(f)
...
for group_name, avg in group_data.items():
    members = []
    if '毛泽东' in group_name:
        members = ['毛泽东']
    elif '历史人物' in group_name:
        members = list(sorted(name_to_modes.keys()))
    elif '现代企业家' in group_name:
        members = list(modern.keys())
    
    for i in range(42):
        high_count = 0
        total = 0
        for name in members:
            modes = name_to_modes.get(name, modern.get(name, []))
            total += 1
            if (i+1) in modes:
                idx = modes.index(i+1) if (i+1) in modes else -1
                if idx != -1 and idx <= 3:
                    high_count += 1
        if total > 0:
            wb['模式覆盖率分析'].cell(row=row_num, column=1, value='历史人物' if '历史人物' in group_name else group_name)
            wb['模式覆盖率分析'].cell(row=row_num, column=2, value=mode_names[i])
            wb['模式覆盖率分析'].cell(row=row_num, column=3, value=high_count)
            wb['模式覆盖率分析'].cell(row=row_num, column=4, value=total)
            wb['模式覆盖率分析'].cell(row=row_num, column=5, value=round(high_count/total, 3))
            row_num += 1
...
for i in range(42):
    ...
    for group_name, avg in group_data.items():
        if avg[i] > max_avg:
            ...
            second_max = avg[i]
    if max_avg > 0:
        wb['群体特有高权重模式'].cell(row=row_num, column=1, value=mode_names[i])
        wb['群体特有高权重模式'].cell(row=row_num, column=2, value=max_group)
        wb['群体特有高权重模式'].cell(row=row_num, column=3, value=round(max_avg, 3))
        wb['群体特有高权重模式'].cell(row=row_num, column=4, value=round(second_max, 3))
        wb['群体特有高权重模式'].cell(row=row_num, column=5, value=round(max_avg - second_max, 3))
        row_num += 1
