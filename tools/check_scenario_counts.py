...
# Check tools directory files (what tests load)
...
print(f'tools/scenarios_zh.json: {len(szh)} entries (list)')
print(f'tools/scenarios_en.json: {len(sen)} entries (list)')
print(f'tools/modes_data.json: {len(modes)} entries')
print(f'tools/code_maps.json CODE_MAP: {len(cm.get("CODE_MAP", {}))} entries')
...
zh_dict = [e for e in szh if is_scenario_entry(e)]
en_dict = [e for e in sen if is_scenario_entry(e)]

print(f'scenarios_zh dict entries: {len(zh_dict)}')
print(f'scenarios_en dict entries: {len(en_dict)}')

h_zh = sum(1 for e in zh_dict if e.get('code', '').startswith(('H-', 'M-')))
h_en = sum(1 for e in en_dict if e.get('code', '').startswith(('H-', 'M-')))
print(f'H/M count ZH: {h_zh}')
print(f'H/M count EN: {h_en}')

p4_zh = sum(1 for e in zh_dict if e.get('code', '').startswith('P4-'))
p4_en = sum(1 for e in en_dict if e.get('code', '').startswith('P4-'))
print(f'P4 count ZH: {p4_zh}')
print(f'P4 count EN: {p4_en}')

p5_zh = sum(1 for e in zh_dict if e.get('code', '').startswith('P5-'))
p5_en = sum(1 for e in en_dict if e.get('code', '').startswith('P5-'))
...
intl_zh = sum(1 for e in zh_dict if any(e.get('code', '').startswith(p) for p in INTL_PREFIXES))
intl_en = sum(1 for e in en_dict if any(e.get('code', '').startswith(p) for p in INTL_PREFIXES))
print(f'International count ZH: {intl_zh}')
print(f'International count EN: {intl_en}')

# Check modes_data structure
print(f'modes_data zh keys: {list(modes.get("zh", {}).keys())[:5]}')
print(f'modes_data en keys: {list(modes.get("en", {}).keys())[:5]}')
print(f'modes_data zh count: {len(modes.get("zh", {}))}')
print(f'modes_data en count: {len(modes.get("en", {}))}')