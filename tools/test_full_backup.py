...
def test_scenarios_zh():
    """测试中文场景加载"""
    assert len(SCENARIOS_ZH) == 858, f"Expected 858, got {len(SCENARIOS_ZH)}"
    h_count = sum(1 for k in SCENARIOS_ZH if k.startswith('H-') or k.startswith('M-'))
    assert h_count == 367, f"Expected 367 historical figures, got {h_count}"
    ...
    assert intl_count == 351, f"Expected 351 international figures, got {intl_count}"
...
def test_scenarios_en():
    """测试英文场景加载"""
    assert len(SCENARIOS_EN) == 858, f"Expected 858, got {len(SCENARIOS_EN)}"
...
def test_modes_data():
    ...
    # 支持扩展模式：原 42 个 + Phase 4 新增
    assert len(zh_modes) >= 42, f"Expected at least 42 zh modes, got {len(zh_modes)}"
    assert len(en_modes) >= 42, f"Expected at least 42 en modes, got {len(en_modes)}"
    for mode_id in zh_modes:
        assert mode_id in en_modes, f"Missing EN mode: {mode_id}"
    ...
    print(f"✓ MODES_DATA: {len(zh_modes)} zh modes + {len(en_modes)} en modes, all bilingual")


def test_code_maps():
    """测试代码映射加载"""
    assert len(CODE_MAP) == 836, f"Expected 836 CODE_MAP, got {len(CODE_MAP)}"
    assert len(CODE_MAP_EN) == 836, f"Expected 836 CODE_MAP_EN, got {len(CODE_MAP_EN)}"
...
def test_modes_coverage():
    ...
    print(f"✓ Mode coverage: {len(used_modes)}/42 modes used")
...
def test_new_batch_figures():
    ...
    all_new = batch_a + batch_b + batch_c + batch_d + batch_e + batch_f + batch_g + batch_h + batch_i
    for code in all_new:
...
def test_cli_list():
    ...
    assert len(lines) == 387, f"Expected 387 scenario lines (A/B/C/D/H/M categories), got {len(lines)}"
