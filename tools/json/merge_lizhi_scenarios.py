#!/usr/bin/env python3
"""Complete Li Zhi (李贽) integration into scenario files."""
...
# === Step 1: Update scenario_tags.json ===
with open(BASE / "scenario_tags.json", "r", encoding="utf-8") as f:
    scenario_tags = json.load(f)

scenario_tags["H-LZ-169"] = lizhi_tags
with open(BASE / "scenario_tags.json", "w", encoding="utf-8") as f:
    json.dump(scenario_tags, f, ensure_ascii=False, indent=2)
print(f"[OK] Updated scenario_tags.json with H-LZ-169 ({len(lizhi_tags['tags'])} tags)")

# === Step 2: Update scenarios_zh.json ===
with open(BASE / "scenarios_zh.json", "r", encoding="utf-8") as f:
    content = f.read()
    
# Find and replace the H-LZ-169 entry in scenarios_zh.json
import re

# Pattern to match the existing H-LZ-169 block
pattern_zh = r'("H-LZ-169":\s*\{[^}]*"description_zh":\s*"\{\u3010\u5f85\u8865\u5145\u3011H-LZ-169\u7684\u63cf\u8ff0\}"\s*\})'
match_zh = re.search(pattern_zh, content)
if match_zh:
    old_entry = match_zh.group(1)
    new_entry_json = json.dumps(lizhi_scenario_zh, ensure_ascii=False, indent=2)
    content = content.replace(old_entry, new_entry_json)
    print(f"[OK] Replaced H-LZ-169 entry in scenarios_zh.json")
else:
    # Try a more flexible pattern
    # Find the H-LZ-169 block and replace it
    start_idx = content.find('"H-LZ-169": {')
    if start_idx >= 0:
        # Find the matching closing brace
        brace_count = 0
        i = start_idx + len('"H-LZ-169": {')
        while i < len(content):
            if content[i] == '{':
                brace_count += 1
            elif content[i] == '}':
        ...
        if prefix.rstrip().endswith(',') and suffix.lstrip().startswith(','):
            suffix = suffix[1:]
        elif not prefix.rstrip().endswith(',') and suffix.lstrip().startswith(','):
            suffix = suffix[1:]
        
        new_json = json.dumps(lizhi_scenario_zh, ensure_ascii=False, indent=2)
        content = prefix.rstrip() + '\n  "H-LZ-169": ' + new_json + suffix.lstrip()
        print(f"[OK] Replaced H-LZ-169 in scenarios_zh.json (flexible match)")
    else:
        print("[WARN] Could not find H-LZ-169 in scenarios_zh.json")

with open(BASE / "scenarios_zh.json", "w", encoding="utf-8") as f:
    f.write(content)
print(f"[OK] Saved scenarios_zh.json")

# === Step 3: Update scenarios_en.json ===
with open(BASE / "scenarios_en.json", "r", encoding="utf-8") as f:
    content = f.read()
    
# Same replacement logic for English
pattern_en = r'("H-LZ-169":\s*\{[^}]*"description_en":\s*"\[TBD\] H-LZ-169 description"\s*\})'
match_en = re.search(pattern_en, content)
if match_en:
    old_entry = match_en.group(1)
    new_entry_json = json.dumps(lizhi_scenario_en, ensure_ascii=False, indent=2)
    content = content.replace(old_entry, new_entry_json)
    print(f"[OK] Replaced H-LZ-169 entry in scenarios_en.json")
else:
    start_idx = content.find('"H-LZ-169": {')
    if start_idx >= 0:
        brace_count = 0
        ...
        new_json = json.dumps(lizhi_scenario_en, ensure_ascii=False, indent=2)
        content = prefix.rstrip() + '\n  "H-LZ-169": ' + new_json + suffix.lstrip()
        print(f"[OK] Replaced H-LZ-169 in scenarios_en.json (flexible match)")
    else:
        print("[WARN] Could not find H-LZ-169 in scenarios_en.json")

with open(BASE / "scenarios_en.json", "w", encoding="utf-8") as f:
    f.write(content)
print(f"[OK] Saved scenarios_en.json")
...
with open(BASE / "scenario_tags.json") as f:
    tags = json.load(f)
    if "H-LZ-169" in tags:
        print(f"  scenario_tags.json: H-LZ-169 OK ({len(tags['H-LZ-169']['tags'])} tags)")
    else:
        print("  scenario_tags.json: H-LZ-169 MISSING")

with open(BASE / "scenarios_zh.json") as f:
    zh = json.load(f)
    if "H-LZ-169" in zh:
        e = zh["H-LZ-169"]
        print(f"  scenarios_zh.json: H-LZ-169 OK (name={e.get('name','')[:30]}..., steps={len(e.get('steps',[]))}, case={'完善' if '完善' in e.get('case','') else 'OK'})")
    else:
        print("  scenarios_zh.json: H-LZ-169 MISSING")

with open(BASE / "scenarios_en.json") as f:
    en = json.load(f)
    if "H-LZ-169" in en:
        e = en["H-LZ-169"]
        print(f"  scenarios_en.json: H-LZ-169 OK (name={e.get('name','')[:40]}..., steps={len(e.get('steps',[]))}, case={'OK' if 'completed' in e.get('case','').lower() or 'Li Zhi' in e.get('case','') else 'check'})")
    else:
        print("  scenarios_en.json: H-LZ-169 MISSING")
