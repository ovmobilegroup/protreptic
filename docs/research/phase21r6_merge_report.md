...
### 3.1 卡片模板口径 vs 实际落盘（偏差登记，逐项如实）

| 卡面模板 | 实际落盘可用 | 处置与依据 |
|---|---|---|
| scenarios_zh/en 各 +100 | zh 30 / en 30（陈云 10 + 纪弦 10 + 傅玄同 10） | CHB/DYC/KKQ/LWH 无场景载荷（落盘卡缺口登记，卡面硬纪律「不可凭空补写」）；YSK 原始场景非库内规范格式（落盘卡未转正）；SQL/WL 场景库内既有 → 仅重指 60 条（未新增） |
| code_maps +10 figure 注册 | 3 件新增注册 + 7 件既有条目重指 | 十件中 7 件在主库已有条目（SQL/CY/WL/CHB/FXT/DYC/KKQ），重复注册会撞键；按「只做引用面随迁」重指 218 叶，另 3 件（YSK/JX/LWH）新增注册；合并后十件登记口径全部归一（figures 212 -> 215） |
| figure_names 登记 10 项 | 10 项已登记 | 零写入（仅复核），登记齐全断言见 §8 E 组 |
| numstat 全 N/0（纯追加） | modes_data +7097/-0 纯追加；code_maps/scenarios/tags 因重指行替换计入删除列 | 删除行 = 同值替换（旧号到新号），非内容删除；四文件「备份前缀逐对象全等」已逐条断言（§5、§8） |
