...
- 本卡回执（stage=v1）：v1 提交 （v1 提交后回填） / 发布仓 （v1 提交后回填） / push （v1 提交后回填）；v2 终版见第四.7节。
...
- 本卡补镜像（v1/v2）见第四.7节回执；发布仓生成时点 HEAD=`15540c302a737a76574d8c38d97a93b731467b6f`。
...
- [R-a] docs/research/phase21r8_w4_names_recon.md（t_24b92f58；研究矩阵 md）：发布仓缺失（待镜像）。
- [R-a] docs/research/phase21r8_w4_names_recon.json（t_24b92f58；研究矩阵 json）：发布仓缺失（待镜像）。
- [R-a] docs/research/phase21w4_fix_manifest.md（t_10a42b2a；处置清单 md）：发布仓缺失（待镜像）。
- [R-a] docs/research/phase21w4_fix_manifest.json（t_10a42b2a；处置清单 json）：发布仓缺失（待镜像）。
- [R-a] docs/research/phase21w6_marker_scan.md（t_51db608d；W6 标记扫描 md）：发布仓缺失（待镜像）。
- [R-a] docs/research/phase21w6_marker_scan.json（t_51db608d；W6 标记扫描 json）：发布仓缺失（待镜像）。
- [QA] docs/qa/phase21w4_qa_report.md（t_8bad8d98；QA 报告（船长 10:58 补镜像令））：发布仓缺失（待镜像）。
- [QA] docs/qa/phase21w4_qa_evidence.json（t_8bad8d98；QA 证据 JSON）：发布仓缺失（待镜像）。
- [QA-见证器] verify_phase21w4_qa_espinosa.py（t_8bad8d98；根级 QA 见证器（v3，可复跑））：发布仓缺失（待镜像）。
...
- stage: v1
- ws_commit_v1: （v1 提交后回填）
- pb_commit_v1: （v1 提交后回填）
- push_v1: （v1 提交后回填）
- ws_commit_v2: （v2 回执终版提交后回填）
- pb_commit_v2: （v2 回执终版提交后回填）
- push_v2: （v2 回执终版提交后回填）
- remote_main: （v2 回填）
- remote_raw: （v2 回填：远端 raw 抽验结果）
- parity_final: （v2 回填：parity 复跑计数与归因）
- verify_result: （v2 回填：verify_w4_names_fix_archive.py 结果）
- report_sha_v1: （v2 回填：报告 v1 文件 sha256 前16）
...
|---|---|---|---|---|
| docs/research/phase21r8_w4_names_recon.md | R-a | 1e66897bf33b90c6 | - | 待镜像/异常 |
| docs/research/phase21r8_w4_names_recon.json | R-a | 54fe78f27e473927 | - | 待镜像/异常 |
| docs/research/phase21w4_fix_manifest.md | R-a | 1886f0bfae22986d | - | 待镜像/异常 |
| docs/research/phase21w4_fix_manifest.json | R-a | c6fb3d908517b6fd | - | 待镜像/异常 |
| docs/research/phase21w6_marker_scan.md | R-a | ab3c08915dd44eec | - | 待镜像/异常 |
| docs/research/phase21w6_marker_scan.json | R-a | 0069cc5c0129c214 | - | 待镜像/异常 |
| docs/qa/phase21w4_qa_report.md | QA | f969ce17c331b115 | - | 待镜像/异常 |
| docs/qa/phase21w4_qa_evidence.json | QA | 2bf1e47f94da39a8 | - | 待镜像/异常 |
| verify_phase21w4_qa_espinosa.py | QA-见证器 | 043f7b956c0b8ea2 | - | 待镜像/异常 |

- 镜像集判定汇总：存在未等同项（生成于镜像前时点为「待镜像」）。

### 5.2 本档四件 sha256（生成时点）

| 文件 | 工作仓 sha256(前16) | 发布仓 sha256(前16) |
|---|---|---|
| docs/research/phase21w4_names_fix_archive_report.md | （本文件；sha 见回执节） | - |
| docs/research/phase21w4_names_fix_archive_evidence.json | - | - |
| verify_w4_names_fix_archive.py | 40a9907da9021c3f | - |
| build_w4_names_fix_archive.py | cf2147d860cfa87b | - |
...
- 工作仓 HEAD = `bb2959fd66df589b9a3244ddd876dbdaf66242fd`（master，本地提交面）。
- 发布仓 HEAD = `15540c302a737a76574d8c38d97a93b731467b6f`（origin/main 面，随本卡镜像推进）。
- 锚点（逐条实测可解）：
