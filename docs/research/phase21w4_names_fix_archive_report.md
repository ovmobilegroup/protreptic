...
- 生成口径：本报告由 build_w4_names_fix_archive.py 逐字段装配（生成时实测），幂等可复跑（--check 归一生成时点观测行——HEAD 与档内 sha——后逐字节比对，打印 IDEMPOTENT）。
...
- 本卡补镜像（v1/v2）见第四.7节回执；发布仓生成时点 HEAD=`cbe34ece7186f0a57f2bcdf695d736879d69a51e`。
...
|---|---|---|
| docs/research/phase21w4_names_fix_archive_report.md | （本文件；sha 见回执节） | 3b1219df78119893 |
| docs/research/phase21w4_names_fix_archive_evidence.json | （刷新件；sha 以两仓 byte-exact 判定与卡面记录为准） | （同左） |
| verify_w4_names_fix_archive.py | ae9388efd4c3ac64 | ae9388efd4c3ac64 |
| build_w4_names_fix_archive.py | 97a81aa85c3dc68f | - |
...
- 工作仓 HEAD = `004332b43637c45447938f7b5b550fa6ce0ff8af`（master，本地提交面）。
- 发布仓 HEAD = `cbe34ece7186f0a57f2bcdf695d736879d69a51e`（origin/main 面，随本卡镜像推进）。
- 锚点（逐条实测可解）：
