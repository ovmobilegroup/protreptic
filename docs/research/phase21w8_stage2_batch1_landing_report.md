...
- **已核条目（quote 3 + variant 22 = 25 条）：不改字。** 逐字/繁简/节引对读结论入账本；见证位 21/25 条随行（`evidence.file` + `snippet`），另 4 条节引型（sent-composite：M-LJY-002 / M-CHE-009 / M-ZXC-004 / M-ZXC-005，无单点见证）按节引档显式登记。素材包未给出处置建议，库内引文与底本差异均为繁简、节引、标点级（对读后命中），无见证支持的改字一律不做。
...
**校订注记（t_2ce1e334，2026-09-24）**：本节原表述「见证位（`evidence.file` + `snippet`）逐条随行」对 25 条已核不成立（实测 21 条带见证、4 条节引型），据 QA 报告 t_e727be57 §3-F4 据实收紧；素材包经同链 F1/F2 修复重算后（t_2ce1e334：ellipsis 分段全命中 + 归一集补 U+FE30/U+3000）计数终值 = 核中 23 条（quote 3 + variant 20）= 见证 20 + 节引 3；null 55→57。本卡（t_b85ae14a）处置动作与账本口径不受影响。
