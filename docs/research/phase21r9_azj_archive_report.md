...
- 工作仓提交：WS1（白名单 5 件：页/生成器/验收脚本/报告 v1/证据 v1）；WS2（v2 回执补记）——SHA 见 v2 补记。
- 发布仓镜像：P1（对齐 WS1，3 件）；P2（对齐 WS2）——SHA 见 v2 补记。
- 推送与远端一致：push 范围与 ls-remote 复核结果见 v2 补记。
