...
前端把 A5 的分片接到了 `/modes` 与 `/figures`。实现与实测：

- **解码 + 分词**: `web/src/api/fulltextSearch.ts` 按 4.3/4.8 的文件格式解码 (varint +
  升序 doc_id 差值 + token 级尾部权重表); 查询期分词与 `crc32(utf8(token[0])) % 16` 分片函数
  与构建脚本逐条一致 —— `tokenizeQuery()` 是 `tools/build_search_index.py` 里 `tokenize()` 的
  等价实现 (NFKC + 连续 CJK 取 2-gram + 拉丁数字小写词, 长度 >= 2)。
- **懒加载与缓存**: 查询长度 >= 2 且防抖 150 ms 后才触发; 一次只拉「查询 token 首字符」命中的
  分片 (实测「幂势既同」= 3 片, 「王阳明」= 2 片), 解出的分片常驻内存 Map, 二次查询零网络。
  分片落在 `data/**`, A4 的 SW 已按 cache-first 缓存, **不需要改 SW**。
- **doc_id 还原**: `web/src/api/modeIndex.ts` 提供 `modes/index-0..7.json` 的共享加载器
  (doc_id = 按序拼接下标), `/modes` 与 `/figures` 共用同一份内存缓存; `/figures` 只在首次检索时
  才加载它 (首屏不加载, 符合 4.4 的时机约定), 代价是首次检索要付模式索引的约 1.1 MB gzip。
- **排序**: 有查询时先按权重和降序 (同分按 doc_id 升序), 索引覆盖不到的 (编号 / 拉丁前缀 /
  场景条目) 用**保留的**子串匹配兜底并追加在后; 无查询时维持原有顺序。类型 / 分类 / 标签筛选
  与分页行为不变。
- **文案 (不得夸大)**: 两个视图的检索框下都写明「按倒排索引匹配……不是语义向量检索」, 并列出
  边界 (拉丁词不做前缀 `decis != decision`、编号与案例正文不在索引内)。结果卡片显示
  「相关度 N」= 命中 token 的权重和, 不是相似度。
- **降级**: `searchFullText()` 在 meta / 分片拉取失败时返回 `null`, UI 回落子串匹配并显示
  「已降级为关键词匹配」; 命中 0 时显示「倒排索引未命中……以下为编号 / 名称 / 领域子串匹配结果」。
  两条路径不静默混合。

实测 (`npx vite preview --base=/protreptic/`, 真实浏览器读 DOM):

```
/modes   幂势既同  -> M-ZUX-001 祖暅 相关度 8 (首条), M-WCY-003 王重阳 2; 提示「倒排索引命中 2 条 · 本次索引分片 3 片」
/modes   王阳明    -> 12 条, M382/M387/M385 相关度 8 居首
/modes   M-ZUX     -> 倒排未命中 (编号不在索引内) -> 子串兜底 10 条
/modes   decis     -> 倒排未命中 -> 子串兜底 29 条 (拉丁前缀是已知边界)
/figures 幂势既同  -> ZuXuan 祖暅 相关度 8 (首条), WCY 王重阳 2
