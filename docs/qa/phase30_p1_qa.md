...
- **线上状态**：线上 bundle 仍是修复前版本（`index-D1DYwfIq.js`），实测线上 `/graph` 下拉仍有 **574/2858** 条串行名称 → **D1 在线上仍未修复，必须重新构建部署后复测归零**。
...
- **D1 是高影响可见缺陷（20% 模式名在 3 个页面串行显示），已在本卡修复并本地验证归零，但线上仍未生效** —— 发布判定的前置条件是「重新构建 + 部署 + 线上复测 D1 = 0」。
- D2 / D3 属数据治理层（名称映射、隔离覆盖不全），不阻塞功能，但会以「人物名看起来是乱码 / 编号」的形式暴露给用户，建议在 C4 终审前或紧随其后修复。
- **建议：可发布（含本卡修复），但不得以当前线上状态直接验收**；C4 终审必须在新一轮 Pages 部署后复测 D1 及本节各项。
